...
bool check(int a, int b)
{
	...
	{
		if (board[i][b]) 
			return false; //Проверка совпадений по горизонтали и вертикали 
	}
	for (int i = 1; i <= a && b - i >= 0; i++)
	{
		if (board[a - i][b - i])
			return false; //Проверка главной диагонали 
	}
	for (int i = 1; i <= a && b + i < 8; i++) 
	{
		if (board[a - i][b + i]) 
			return false; //Проверка побочной диагонали 
	}
	return true;
}
